            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                  ...
                  <x-flash-message status="session('status')" />
                    <form class="" method="post" action="{{ route('owner.products.update', ['product' => $product->id ]) }}">
											@csrf
                      @method('put')
											<div class="-m-2">
                            ...
                            <div class="mt-4 mb-4">
                              ...
                              <input type="radio" name="is_selling" value="0" class="mr-2" @if($product->is_selling === 0){ checked } @endif>停止中
												...
												<div class="p-2 w-1/2 mx-auto flex justify-between mt-4">
													<button type="button" onclick="location.href='{{ route('owner.products.index') }}'" class="bg-gray-200 bg-yellow-500 border-0 py-2 px-8 focus:outline-none hover:bg-gray-400 rounded text-lg">戻る</button>
													<button type="submit" class="text-white bg-yellow-400 border-0 py-2 px-8 focus:outline-none hover:bg-yellow-500 rounded text-lg">更新する</button>
												</div>
											</div>
                    </form>
                    <form id="delete_{{$product->id}}" action="{{ route('owner.products.destroy', ['product' => $product->id]) }}" method="post">
                      ...
                      <div class="p-2 w-full flex justify-around mt-32">
                        <a href="#" data-id="{{ $product->id }}" onclick="deletePost(this)" class="text-white bg-gray-200 border-0 py-2 px-4 focus:outline-none hover:bg-gray-400 rounded">削除する</a>
                      </div>
                    </form>
                </div>
            </div>
