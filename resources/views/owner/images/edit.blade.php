        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form class="" method="post" action="{{ route('owner.images.update', ['image' => $image->id ]) }}">
											@csrf
                      @method('put')
											<div class="-m-2">
                            ...
                            <div class="w-32">
                              <x-thumbnail :filename="$image->filename" type="products" />
												...
												<div class="p-2 w-1/2 mx-auto flex justify-between mt-4">
													<button type="button" onclick="location.href='{{ route('owner.images.index') }}'" class="bg-gray-200 bg-yellow-500 border-0 py-2 px-8 focus:outline-none hover:bg-gray-400 rounded text-lg">戻る</button>
                    ...
                    <form id="delete_{{$image->id}}" action="{{ route('owner.images.destroy', ['image' => $image->id]) }}" method="post">
                      ...
                      <div class="p-2 w-full flex justify-around mt-32">
                        <a href="#" data-id="{{ $image->id }}" onclick="deletePost(this)" class="text-white bg-gray-200 border-0 py-2 px-4 focus:outline-none hover:bg-gray-400 rounded">削除する</a>
                      </div>
                    </form>
                </div>
            </div>
