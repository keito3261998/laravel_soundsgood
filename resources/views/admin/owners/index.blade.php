								      ...
								      <table class="table-auto w-full text-left whitespace-no-wrap">
								        ...
								        <tbody>
								          ...
								          <tr>
								            ...
								            <td class="md:px-4 py-3 text-right">
								              <button onclick="location.href='{{ route('admin.owners.edit', ['owner' => $owner->id]) }}'" class="text-white bg-yellow-200 border-0 py-2 px-4 focus:outline-none hover:bg-yellow-400 rounded">編集</button>
								            </td>
														<form id="delete_{{$owner->id}}" action="{{ route('admin.owners.destroy', ['owner' => $owner->id]) }}" method="post">
															@csrf
															@method('delete')
															<td class="md:px-4 py-3 text-right">
									              <a href="#" data-id="{{ $owner->id }}" onclick="deletePost(this)" class="text-white bg-gray-200 border-0 py-2 px-4 focus:outline-none hover:bg-gray-400 rounded">削除</a>
									            </td>
														</form>
								          </tr>
													@endforeach
